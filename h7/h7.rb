...
class ULVar < ULTerm
    ...
    def prettify
        posNum = [ 'a','b','c','d','e','f','g','h','i','j','k','l','m','n','o','p','q','r','s','t','u','v','w','x','y','z']
        amtZ = @index  / 26
        char = @index % 26
        return posNum[25] * amtZ +  posNum[char]
    end
...
class ULAbs < ULTerm
    ...
    def prettify
        return "lambda " + @t.helper($value) + " . " + @t.prettify
    end
...
class ULApp < ULTerm
    ...
    def prettify
        return "(" + @t1.prettify  + ") (" + @t2.prettify  + ")"
    end
end
